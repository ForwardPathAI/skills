...
```
- [ ] Step 0: Resolve root team + sub-teams in scope
...
Merge all results into one map keyed by project `id`. If the same project appears from multiple team queries, keep a single entry.
...
Record `name`, `url`, `status.name`, and which scope team(s) the project is on (`project.teams`) for the report appendix.
...
| New sub-team added under Engineering | Pick it up via `parent` on `list_teams`, or fallback pod list — do not hardcode only Pod 1 / Pod 2 if `parent` is available. |
